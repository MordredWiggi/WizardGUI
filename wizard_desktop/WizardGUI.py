...
class MplCanvas(FigureCanvasQTAgg):
    def __init__(
        self, parents=None, width=5, height=4, title=None, xlabel=None, ylabel=None
    ):
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.layout_settings.addWidget(
            self.label_names_players,
            0,
            0,
            alignment=QtCore.Qt.AlignmentFlag.AlignCenter,
        )
        self.layout_settings.addWidget(
            self.lineedit_names_players,
            0,
            1,
            alignment=QtCore.Qt.AlignmentFlag.AlignCenter,
        )
        ...
        self.layout_standings.addWidget(
            self.label_players, 0, 0, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
        )
        self.layout_standings.addWidget(
            self.label_said, 0, 1, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
        )
        self.layout_standings.addWidget(
            self.label_achieved, 0, 2, alignment=QtCore.Qt.AlignmentFlag.AlignCenter
        )
        self.wdg_standings.setLayout(self.layout_standings)

        self.button_round_done = QtWidgets.QPushButton(
            "Runde beendet", self, clicked=self.roundDone
        )
    ...
    def playerNamesChanged(self):
        ...
        self.list_player_names = (
            self.lineedit_names_players.text().replace(" ", "").split(",")
        )
        ...
        for i in range(0, len(self.list_player_names)):
            self.layout_standings.addWidget(
                self.list_labels_player_names[i],
                i + 1,
                0,
                alignment=QtCore.Qt.AlignmentFlag.AlignCenter,
            )
            self.layout_standings.addWidget(
                self.list_lineedits_said[i],
                i + 1,
                1,
                alignment=QtCore.Qt.AlignmentFlag.AlignCenter,
            )
            self.layout_standings.addWidget(
                self.list_lineedits_achieved[i],
                i + 1,
                2,
                alignment=QtCore.Qt.AlignmentFlag.AlignCenter,
            )
        self.layout_standings.addWidget(
            self.button_round_done, len(self.list_player_names) + 1, 0, 1, 3
        )
    ...
    def roundDone(self):
        ...
        for i in range(len(self.list_player_names)):
            ...
            if said == achieved:
                ...
                self.list_arrs[i].append(
                    self.list_arrs[i][-1] - 10 * np.abs(said - achieved)
                )
        ...
        for i_arr, arr in enumerate(self.list_arrs):
            self.plot.axes.plot(
                self.list_rounds, arr, marker="o", label=self.list_player_names[i_arr]
            )
            max_i = np.argmax(arr)
            self.plot.axes.plot(
                self.list_rounds[max_i],
                arr[max_i],
                marker="d",
                color="black",
                markersize=10,
